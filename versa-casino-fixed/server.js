...
// ВАЖНО: на Vercel файловая система доступна на запись только в /tmp,
// и она НЕ сохраняется между запусками функции (serverless = без постоянного диска).
// Поэтому в проде (Vercel) БД будет создаваться заново при каждом "холодном старте",
// и данные пользователей будут теряться. Для реального продакшена нужен внешний БД-сервис
// (Vercel Postgres, Turso, Supabase, PlanetScale и т.п.) — см. пояснение в чате.
...
// На Vercel этот файл не запускается напрямую как сервер —
// его подключает /api/index.js, а Vercel сам оборачивает app в serverless-функцию.
// Поэтому app.listen() вызываем только при локальном запуске (node server.js).
